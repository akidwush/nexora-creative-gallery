...
export default function WorkDetailPage() {
  ...
  useEffect(() => {
    let active = true;
    let invalidWorkFrame: number | null = null;
    ...
    if (workId) {
      ...
      invalidWorkFrame = window.requestAnimationFrame(() => {
        if (!active) return;
    ...
    return () => {
      active = false;
      if (invalidWorkFrame !== null) {
        window.cancelAnimationFrame(invalidWorkFrame);
      }
    };
  }, [workId]);
  ...
  return (
    <main className={styles.page}>
      <header className={styles.header}>
        ...
        <Link className={styles.brand} href="/" aria-label="Nexora Creative Gallery">
